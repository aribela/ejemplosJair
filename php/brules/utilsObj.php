...
// Imp. 23/05/22 Carlos => Obtener un arreglo por la clave elegida (Pueder ser objeto o arreglo)
function obtArrayPorValor($col, $valor, $unico=false, $obtString=false){
    $str = implode(',', array_column($col, $valor));
    $arr = explode(",", $str);
    if($unico){
        $arr = array_unique($arr); //Arreglo unico de ids de gerentes
    }
...
$msg = '<h1 class="titulo">Agregar pregunta para la encuesta <span class="empresa">'.$company.'</span> <!-- en el idioma <span class="idioma">'.$language.'</span>-->

<span class="pull-right"><a id="btnAyudaweb" onclick="mostrarAyuda(\''.$ayuda.'\')" href="#fancyAyudaWeb"><img src="../images/icon_ayuda.png" width="20px"></a></span>
</h1>';
        
    return $msg;
}

function generaHtmlForm2(datosPanelObj $datosPanel, $campos = array()){
  $html = '';
  $htmlCampos = '';
  ...
  foreach ($campos as $campo) {
    ...
    switch($campo->type){
      case 'hidden': 
        $htmlInput .= '<input class="form-control '.$campo->claseInput.'" type="'.$campo->type.'" name="'.$campo->nameid.'" id="'.$campo->nameid.'" value="'.$campo->value.'" >';
      break;
      case 'text': 
        $valor = (isset($campo->value))?$campo->value:'';
        $htmlInput = '<input class="form-control '.$campo->claseInput.'" type="'.$campo->type.'" name="'.$campo->nameid.'" id="'.$campo->nameid.'" value="'.$valor.'"  '.$atributos.' >';
      break;
    }

    $htmlCampos .= '
        <div class="row '.$campo->claseRow.'">
            <div class="'.$campo->claseDivLabel.' text-right">
              <label class="'.$campo->claseLabel.' for="'.$campo->nameid.'">'.$campo->label.' </label>
            </div>
            <div class="'.$campo->claseDivInput.'">
                '.$htmlInput.'
            </div>
        </div>
    ';
  ...
  <div class="panel panel-warning">
      <div class="panel-heading">'.$datosPanel->titulo.'</div>
...
function generaHtmlForm($campos, $cols = array("label_xs"=>"4", "label_md"=>"4", "input_xs"=>"8", "input_md"=>"8"), $htmlExtra = ''){
  $html = '';
  // echo $claseRow;
  foreach ($campos as $campo) {
    $claseRow = (isset($campo["claseRow"]))?$campo["claseRow"]:'';
    if($campo["type"] == 'hidden'){
      $html .=  '<input class="form-control '.$campo["class"].'" type="'.$campo["type"].'" name="'.$campo["nameid"].'" id="'.$campo["nameid"].'" value="'.$campo["value"].'" >';
    }else{
        $atributos = '';
        $input = '';
        $dataInput = '';
        ...
        elseif($campo["type"] == 'select'){
          ...
          $input = '<input class="form-control '.$campo["class"].'" type="'.$campo["type"].'" name="'.$campo["nameid"].'" id="'.$campo["nameid"].'" '.$atributos.' '.$dataInput.'>';
...
function obtenerCampoGral($nameId = '', $valor = '', $label = '', $claseCampo = '', $objeto = array(), $clasesPreguntas = false, $evento = '', $formula = ''){
  ...
  //evento onchange para guardado automatico de respuestas cuestionario Jair 29/3/2022
  $claseCampoSearch = " ".$claseCampo;
  if(strpos($claseCampoSearch, 'fiscales') || strpos($claseCampoSearch, 'comercial') || strpos($claseCampoSearch, 'certificaciones') || strpos($claseCampoSearch, 'confirmacion')){
    $data["onchange"] = "guardarCampoCues('".$nameId."', 'perfil')"; 
  }

  if(strpos($claseCampoSearch, 'cuestionario')){
    $data["onchange"] = "guardarCampoCues('".$nameId."', 'cuestionario_1')"; 
  }

  if(strpos($claseCampoSearch, 'acuerdo')){
    $data["onchange"] = "guardarFormProveedor(\'acuerdo\', \'acuerdo\')"; 
  }
  ...
  $atributos = array();
  if($formula != ''){
    $atributos[] = array("atributo"=>"data-formula", "valor"=>$formula);
  }

  $claseRow = '';
  $arrClasesRow = array();
  if(strpos($claseCampo, 'oculto2')){
    $arrClasesRow[] = "oculto2";
  }

  if(strpos($claseCampo, 'si_ctpat')){
    $arrClasesRow[] = "si_ctpat";
  }

  if(strpos($claseCampo, 'no_ctpat')){
    $arrClasesRow[] = "no_ctpat";
  }

  if(strpos($claseCampoSearch, 'bancarios2')){
    $arrClasesRow[] = "rowbancarios2";
  }

  $claseRow = implode(" ", $arrClasesRow);

  $valor = (isset($objeto->{$nameId}))?$objeto->{$nameId}:$valor;
  if(strpos($claseCampoSearch, 'referencias') || strpos($claseCampoSearch, 'operativos')){
    $arrNameId = explode("_", $nameId);
    $valor = (isset($objeto->{$arrNameId[0]}))?$objeto->{$arrNameId[0]}:$valor;
  }

  $claseRequired = ($nameId != 'emailEnvioTest')?'required':'';
  $readonly = (strpos($claseCampoSearch, 'promgrupo') || strpos($claseCampoSearch, 'prom_eval'))?true:false;
  $claseCampo = $claseRequired.' '.$claseCampo;
  //campo
  $arrCampoRazon = array(
      array("nameid"=>$nameId, "type"=>"text", "class"=>"form-control ".$claseCampo, "readonly"=>$readonly, "label"=>$label, "datos"=>array(), "value"=>$valor, "dataInput"=>$data, 'claseRow'=>$claseRow, "atributos"=>$atributos),
  );

  if($clasesPreguntas){
    $cols = array("label_xs"=>"6", "label_md"=>"6 cont_preg", "input_xs"=>"4", "input_md"=>"4 cont_resp");
  }else{
    $cols = array("label_xs"=>"6", "label_md"=>"6", "input_xs"=>"4", "input_md"=>"4");
  }

  return generaHtmlForm($arrCampoRazon, $cols);
  
}

function obtenerCampoSelectGral($datosObj = array(), $campoValor = '', $campoNombre = '', $nameId = '', $label = '', $value = '', $claseCampo = '', $objeto = array(), $data = array(), $multipleValues = '', $clasesPreguntas = false, $evento = '' ){
  ...
  $claseCampoSearch = " ".$claseCampo;
  $arrDatos = array();
  $arrDatos[] = array("valor"=>"", "texto"=>"Seleccione...");
  if(count($datosObj) > 0){
    foreach ($datosObj as $itemRes) {
      $atributos = array();
      if(strpos($claseCampoSearch, 'evaluacion')){
        $atributos[] = array("atributo"=>"data-porcentaje", "valor"=>$itemRes->porcentaje); 
      }
        $arrDatos[] = array("valor"=>$itemRes->{$campoValor}, "texto"=>$itemRes->{$campoNombre}, "atributos"=>$atributos);
    }
  }
  ...
  if($nameId == 'certCtpatOpc'){
    $data["onchange"] .= "cambiaCtpatOpc(this.value);";
  }
  ...
  if(strpos($claseCampo, 'bancarios2')){
    $arrClasesRow[] = "rowbancarios2";
  }

  $claseRow = implode(" ", $arrClasesRow);
  $value = (isset($objeto->{$nameId}))?$objeto->{$nameId}:$value;
  if(strpos($claseCampoSearch, 'referencias') || strpos($claseCampoSearch, 'operativos')){
    $arrNameId = explode("_", $nameId);
    $value = (isset($objeto->{$arrNameId[0]}))?$objeto->{$arrNameId[0]}:$value;
  }
  ...
  //campo
  $arrCampoGral = array(
      array("nameid"=>$nameId, "type"=>"select", "class"=>"form-control selectpicker ".$claseCampo, "readonly"=>false, "label"=>$label, "datos"=>$arrDatos, "value"=>$value, "dataInput"=>$data, "multivalue"=>$multipleValues, 'claseRow'=>$claseRow),
  );
  ...
  return generaHtmlForm($arrCampoGral, $cols);
  
}

function campoALabel($string = ''){
  return array_reduce(
    str_split($string),
    function($c, $v) {
